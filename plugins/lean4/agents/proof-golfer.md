...
4. **Lemma replacement search** (if search_mode ≠ off):
   ...
   - If replacement needs a statement change → stop and report it (`next_action = redraft`; golf never changes statements). If it needs a multi-file or strategy-level refactor → stop, hand off to `/lean4:refactor`. Hand off to axiom-eliminator only for axiom/assumption hygiene
...
- If replacement needs a statement change → report it, never apply it (`next_action = redraft`); if it needs a multi-file or strategy-level refactor → hand off to `/lean4:refactor`; hand off to axiom-eliminator only for axiom/assumption hygiene, never as a generic "bigger surgery" target
